...
#' Definitions 
#' 
#' For more information, please see Asness, Frazzini and Pedersen (2014), 
#' “Quality Minus Junk," 
#' http://papers.ssrn.com/sol3/papers.cfm?abstract_id=2312432. 
#' 
#' References
#' Asness, Cliff, Andrea Frazziniand Lasse H. Pedersen, 2014, "Quality Minus Junk," 
...
#'  
#'  Cutoffs are not a differentiator – We both use 30th and 70th NYSE 
#'  percentiles for high/low and the median NYSE breakpoint for small/big.
#'  
...
#'  Data Sources
#'  
#'  The underlying data may differ from Asness, Frazzini and Pedersen (2014), 
#'  but the message of the paper is preserved. Except where noted, data is 
#'  updated and refreshed monthly in order to utilize the best available data. 
#'  The data sources themselves may differ in order to allow for continued 
#'  updating. In addition, the underlying data may be improved and expanded as 
#'  it becomes available. Correlations with the original paper data are quite 
#'  high.
#'  
#'  Pricing and accounting data are from the union of the CRSP tape and the 
#'  Compustat/XpressFeed Global database. The domestic data include all 
#'  available common stocks in the merged CRSP/XpressFeed data. The 
#'  international data include all available common stocks on the 
#'  Compustat/XpressFeed Global database for 23 developed markets. Individual 
#'  issues are assigned to the corresponding market based on the location of the 
#'  primary exchange. For companies traded in multiple markets we use the primary 
#'  trading vehicle identified by Compustat/XpressFeed. All portfolio returns are 
#'  in USD and do not include any currency hedging. Excess returns are above the 
#'  U.S. Treasury bill rate. One set of portfolios is formed in each country and 
#'  aggregates are computed by weighting each country's portfolio by the country's 
#'  total lagged (t-1) market capitalization.
#'  
#'  Disclosures
#'  
...
#'  not responsible for any errors or omissions, regardless of the cause, for 
