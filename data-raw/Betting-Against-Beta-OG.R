...
BAB.Factors<- openxlsx::read.xlsx(Betting.Against.Beta.url, sheet=1, startRow=10, colNames=TRUE, detectDates = TRUE)
...
BAB.Factors$Date <- as.Date(paste0(substr(BAB.Factors$Date, 1, 4), "-", substr(BAB.Factors$Date, 5, 6), "-01"))
...
save(BAB.Factors, file = paste0("data/BAB.Factors.OG.RData"), compress = "xz", compression_level = 9)
